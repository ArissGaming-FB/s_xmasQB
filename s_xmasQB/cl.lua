...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
  Wait(1000) --Lets wait until player has really loaded in
...
function DrawStuff()
	CreateThread(function()
		...
		while spawned do
    		...
    		local treeObj = GetClosestObjectOfType(pos.x, pos.y, pos.z, 1.5, `prop_xmas_tree_int`, false, false, false)
    		if treeObj ~= 0 then
			--local coords = GetEntityCoords(PlayerPedId())
			--local treeObj, distance = QBCore.Functions.GetClosestObject(coords)
			--local closestTree, distance = ESX.Game.GetClosestObject('prop_xmas_tree_int')
			--if distance <= 5 and treeObj == `prop_xmas_tree_int` then
				sleep = 0
				DrawText3D(GetEntityCoords(treeObj) + vector3(0.0, 0.0, 0.0+1.4), 'Claim your present! [~g~E~s~]', 0.3)
				if IsControlJustReleased(0,38) and not claimed then --Change key to what u want
					claimed = true
					TriggerServerEvent('s_xmas:claim', GetEntityCoords(treeObj), GetEntityCoords(plyped))
